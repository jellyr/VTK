...
void vtkMoleculeMapper::UpdateBondGlyphPolyData()
{
  ...
  for (vtkIdType bondInd = 0; bondInd < numBonds; ++bondInd)
    ...
    // TODO Remove/replace when/if subtraction is added back for vtkVector
//    bondVec = pos2 - pos1;
    bondVec[0] = pos2[0] - pos1[0];
    bondVec[1] = pos2[1] - pos1[1];
    bondVec[2] = pos2[2] - pos1[2];
    // end vtkVector TODO
    bondLength = bondVec.Normalize();
    // - Center of bond for translation
    // TODO vtkVector addition
...
//          initialDisp = -delta;
    ...
    if (this->UseMultiCylindersForBonds)
      {
      // TODO vtkVector additon
//      cylinderCenter = bondCenter + initialDisp;
      cylinderCenter[0] = bondCenter[0] + initialDisp[0];
      cylinderCenter[1] = bondCenter[1] + initialDisp[1];
      cylinderCenter[2] = bondCenter[2] + initialDisp[2];
      // end vtkVector addition
      }
    else
      {
